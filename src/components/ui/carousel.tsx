...
const CarouselPrevious = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(
  ({ className, variant = "ghost", size = "icon", ...props }, ref) => {
    ...
    // In RTL mode, "previous" visually (left arrow) should scroll to next items
    const handleClick = isRtl ? scrollNext : undefined;
    const canScroll = isRtl ? canScrollNext : undefined;

    // Fallback to original behavior for non-RTL
    ...
    return (
      <Button
        ...
        size={size}
        className={cn(
          "absolute h-10 w-10 md:h-12 md:w-12 bg-transparent border-0 hover:bg-transparent",
        ...
        {...props}
      >
        <ArrowLeft className="h-6 w-6 md:h-8 md:w-8 text-foreground/70 hover:text-foreground transition-colors" />
...
const CarouselNext = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(
  ({ className, variant = "ghost", size = "icon", ...props }, ref) => {
    const { orientation, scrollNext, canScrollNext, scrollPrev, canScrollPrev, opts } = useCarousel();
    const isRtl = opts?.direction === "rtl";
    ...
    return (
      <Button
        ...
        {...props}
      >
        <ArrowRight className="h-6 w-6 md:h-8 md:w-8 text-foreground/70 hover:text-foreground transition-colors" />
